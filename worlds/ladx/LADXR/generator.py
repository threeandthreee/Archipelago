...
def generateRom(base_rom: bytes, args, data: Dict):
    ...
    if options["gfxmod"] != "Link":
        patches.aesthetics.gfxMod(rom, os.path.join("data", "sprites", "ladx", f"{options['gfxmod']}.bdiff"))
    ...
    elif isinstance(data["world_setup"]["goal"], str) and data["world_setup"]["goal"].startswith("="):
        patches.goal.setSpecificInstruments(rom, [int(c) for c in data["world_setup"]["goal"][1:]])
    else:
    ...
    if (options["warps"] != Options.Warps.option_vanilla) or (options.get("warp_improvements")):
        patches.core.addWarpImprovements(rom, (options["warps"] == Options.Warps.option_improved_additional) or (options.get("additional_warp_points")))
