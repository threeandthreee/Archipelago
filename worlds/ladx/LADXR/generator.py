...
def generateRom(base_rom: bytes, args, patch_data: Dict):
    ...
    # Needs to be done after setting the start location.
    patches.core.warpHome(rom,
        options["entrance_rules"] == Options.EntranceRules.option_chaos or
        options["entrance_rules"] == Options.EntranceRules.option_insane or
        options["entrance_rules"] == Options.EntranceRules.option_madness)
